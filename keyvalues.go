...
// ValuesForKey returns all values in Map, 'mv', associated with a 'key'. If len(returned_values) == 0, then no match.
...
// ValuesForPath retrieves all values for a path from the Map.  If len(returned_values) == 0, then no match.
...
// PathsForKey gets all paths through Map, 'mv', (in dot-notation) that terminate with the specified key.
...
// ValueForPathString returns the first found value for the path as a string.
